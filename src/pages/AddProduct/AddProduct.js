...
export default function AddProduct() {
    ...
    // single function for multiple fetch
    async function fetchApiValue(extension, setState) {

        const response = await axios.get(`https://bootcamp.akbolat.net/${extension}`);

        const categoryArray = response?.data?.reduce((accumulator, currValue) => [...accumulator, currValue.name], [])
    ...
    useEffect(() => {
        ...
        fetchApiValue('using-statuses', setUsingStatuses)
    }, [])
    ...
    const postProduct = async (userData) => {
        ...
        const address = 'https://bootcamp.akbolat.net/products';
